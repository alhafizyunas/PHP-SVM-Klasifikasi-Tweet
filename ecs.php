...
function ECS($kata){
	...
	if (cekKamus($kata)) { // Cek Kamus
		...
		//Pergunakan "" untuk menandakan tidak ditemukan kata dasar
		return ""; //"" ATAU $kataAsal;
	}
}
...
function cekKamus($kata){
	global $array_kamus;
	if(!empty($array_kamus[$kata])){
		return true; // True jika ada
	}else{
		return false; // jika tidak ada FALSE
	}
}
/*============= Stemming dengan Metode ECS Algorithm ===============================*/
...
/*------------ Awalan “te-”, ---------------------------------------------*/
		if(preg_match('/^(te)\S{1,}/',$kata)){ // Jika awalan “te-”,
			...
			if(preg_match('/^(ter)[^aiueor]er[^aiueo]\S{1,}/',$kata)){ // aturan  35 
				$__kata = preg_replace('/^(ter)/','',$kata);
				if(cekKamus($__kata)){			
					 return $__kata; // Jika ada balik
				}
