...
<?php
	try {
		...
		$sql = "SELECT * FROM unduh3 LIMIT 11";
		...
		while (($obj = $stmt->fetch()) == true) {
			...
			?>
			<td align=center>
			<select onchange="ubahLabel(this.value, '<?= $obj->no ?>', 'label')">
				<option <?= ($obj->label == '1') ? 'selected' : '' ?>>1</option>
				<option <?= ($obj->label == '-1') ? 'selected' : '' ?>>-1</option>
			</select>
			</td>
			<td>
			<select onchange="ubahLabel(this.value, '<?= $obj->no ?>', 'ket')">
				<option <?= ($obj->Keterangan == 'Training') ? 'selected' : '' ?>>Training</option>
				<option <?= ($obj->Keterangan == 'Testing') ? 'selected' : '' ?>>Testing</option>
			</select>	
			</td>
			<?php 
			echo '</tr>';
