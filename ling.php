...
// echo " [" . $result->created_at . "] " . $result->user->screen_name . ": " .
$result->text . "<br/><br/>";
$tweet = $result->text;
...
$tweet = preg_replace ('/[#]+([A-Za-z0-9-_]+)/i','',$tweet);
echo $tweet."<br>";
$que = mysqli_query($koneksi,"SELECT * FROM twec WHERE tweet");
if($que){
$cek_exist = mysqli_fetch_array($que);
echo count($cek_exist)."<br>";
if(count($cek_exist)==0){
$q = mysqli_query($koneksi, "INSERT INTO twec ('','tweet','','','') VALUES('',".$tweet."','','','')");
}
}
}
}else{
